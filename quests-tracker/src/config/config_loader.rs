...
pub fn get_adventurers_secret_env() -> Result<AdventurersSecret> {
    ...
    Ok(AdventurersSecret {
        secret: std::env::var("JWT_ADVENTURER_SECRET")
            .expect("JWT_ADVENTURER_SECRET URL is invalid"),
        refresh_token: std::env::var("JWT_ADVENTURER_REFRESH_SECRET")
            .expect("JWT_ADVENTURER_REFRESH_SECRET URL is invalid"),
    })
}
...
pub fn get_guild_commanders_secret_env() -> Result<GuildCommandersSecret> {
    ...
    Ok(GuildCommandersSecret {
        secret: std::env::var("JWT_GUILD_COMMANDER_SECRET")
            .expect("JWT_GUILD_COMMANDER_SECRET URL is invalid"),
        refresh_token: std::env::var("JWT_GUILD_COMMANDER_REFRESH_SECRET")
            .expect("JWT_GUILD_COMMANDER_REFRESH_SECRET URL is invalid"),
    })
}
